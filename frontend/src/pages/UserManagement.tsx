      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-slate-500">{t.manageUsersRolesDepartments}</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-3">
            <div className="text-xs text-slate-500">{t.totalUsers}</div>
          ...
          <div className={`rounded-xl shadow-sm border p-3 ${Object.keys(evalSummaries).length > 0 ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-200'}`}>
            <div className="text-xs text-slate-500">{t.performanceAlerts}</div>
            <div className={`text-2xl font-bold ${Object.keys(evalSummaries).length > 0 ? 'text-orange-600' : 'text-slate-800'}`}>
              {Object.keys(evalSummaries).length}
            </div>
          </div>
          <div className={`rounded-xl shadow-sm border p-3 ${Object.values(evalSummaries).filter(s => s.disciplinaryStatus === 'TERMINATION_RISK' || s.disciplinaryStatus === 'FINE').length > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
            <div className="text-xs text-slate-500">{t.criticalFineTermination}</div>
            <div className={`text-2xl font-bold ${Object.values(evalSummaries).filter(s => s.disciplinaryStatus === 'TERMINATION_RISK' || s.disciplinaryStatus === 'FINE').length > 0 ? 'text-red-600' : 'text-slate-800'}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.user}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.role}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.departments}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.status}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.performance}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.lastLoginCol}</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 uppercase tracking-wider">{t.actions}</th>
                </tr>
              </thead>
